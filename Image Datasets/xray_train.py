...
--------------------------
'''
def train_one_epoch_mil(model, optimizer, scheduler, train_loader, criterion, device, cfg):
    model.train()
    ...
    for i, batch in enumerate(train_loader):
        inputs = batch['study'].to(device).float()
    ...
    metrics = compute_metrics_multi(np.concatenate(target_ls), np.concatenate(output_ls), cfg)
    return np.mean(losses), metrics 

def train_one_epoch(model, optimizer, scheduler, train_loader, criterion, device, cfg):
    model.train()
...
def evaluate_mil(model, test_loader, criterion, device, cfg):
    model.eval()
...
def compute_metrics_multi(y_true, y_pred, cfg):
    mets = dict()
...
def main(cfg):
    ...
    for i, (train_index, val_index) in enumerate(skf.split(targets_df, targets_df['fracture_type'])):
        ...
        if not cfg['mil']:
            ...
            if cfg['net_kwargs']['pooling_method'] == 'stan':
                model = StandardMILNet(encoder, num_features=num_features, num_out=cfg['net_kwargs']['num_out'], dropout=cfg['net_kwargs']['drop_rate'], debug=cfg['net_kwargs']['debug'])
            else:
        ...
        for epoch in range(epochs):
            ...
            if cfg['mil']:
                train_loss, train_metrics = train_one_epoch_mil(model, optimizer, lr_scheduler, train_loader, criterion, device, cfg)
            else:
                train_loss, train_metrics = train_one_epoch(model, optimizer, lr_scheduler, train_loader, criterion, device, cfg)
            ...
            # evaluate
            if cfg['mil']:
                val_loss, val_metrics = evaluate_mil(model, val_loader, criterion, device, cfg)
            else:
